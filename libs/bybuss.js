			...
			if (req.readyState == 4)
			{
				switch ( req.status )
				{
					case 200:
						...
						var answer = req.responseText.replace(/ kl. /gi, ' kl ');
					...
					case 404:
						$('#result').html('Kunne ikke nå tjenesten. Vennligst prøv på nytt.<br><br>Kontakt <a href="mailto:mail@trimn.net">utvikler</a> om problemet vedvarer. Takk!');
						break;
						
					default:
						$('#result').html("Det oppsto en uventet feil. Vennligst prøv på nytt.");
				}
				
			}
		}
		req.open('POST', 'http://www.atb.no/xmlhttprequest.php?service=routeplannerOracle.getOracleAnswer&question=' + question, false);
		req.send(null);
	}
}
